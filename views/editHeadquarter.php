        ...
        <div class="col-sm-8 col-md-6 border border-dark rounded p-4">
            <form>
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" value=<?= htmlspecialchars($headquarter['ID']); ?> name="company_id">
                </div>
                <div class="mb-3">
                    <label for="exampleInputPassword1" class="form-label">Consumer ID</label>
                    <input type="text" class="form-control" value=<?= htmlspecialchars($headquarter['Email']); ?> name="consumer_id" required>
                </div>
                <div class="mb-3">
                    <label class="form-label" for="exampleCheck1">Fuel type</label>
                    <input type="text" name="fuel_type" value=<?= htmlspecialchars($headquarter['Nb_of_employees']); ?> class="form-control" required>
                </div>
                <div class="mb-3">
                    <label class="form-label" for="exampleCheck1">Fuel amount</label>
                    <input type="text" name="fuel_amount" value=<?= htmlspecialchars($headquarter['Country']); ?> class="form-control" required>
                </div>
                <div class="mb-3">
                    <label class="form-label" for="exampleCheck1">Payment date</label>
                    <input type="text" name="payment_date" value=<?= htmlspecialchars($headquarter['City']); ?> class="form-control" required>
                </div>
                <div class="mb-3">
                    <label class="form-label" for="exampleCheck1">Payment method</label>
                    <input type="text" name="payment_method" value=<?= htmlspecialchars($headquarter['Street']); ?> class="form-control" required>
                </div>
                <div class="mb-3">
                    <label class="form-label" for="exampleCheck1">Currency</label>
                    <input type="text" name="currency" value=<?= htmlspecialchars($headquarter['Building']); ?> class="form-control" required>
                </div>
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $result = $headquarterController->updateheadquarter(
        $company_id, $consumer_id, $fuel_type, $fuel_amount, $payment_date, $payment_method, $currency, $price
    );

    if($result) {
        echo "headquarter has been updated";
    } else {
        echo "Error updating headquarter";
    }
}